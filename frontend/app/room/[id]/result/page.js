...
import useResultPage from '@/features/result/hooks/useResultPage';
import RankBadge from '@/features/result/components/RankBadge';
import DonationBanner from '@/features/result/components/DonationBanner';
import RematchModal from '@/features/result/components/RematchModal';
import { getPlayerDamage } from '@/features/result/utils/resultCalc';
...
export default function ResultPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--kn-bg)', color: 'var(--kn-text)', display: 'flex', flexDirection: 'column' }}>
      ...
      {rematchModal && (
        <RematchModal nextSessionName={rematchModal.name} onJoin={handleRematchJoin} onLater={handleRematchLater} />
      )}

      {/* 헤더 */}
      <div style={{ background: 'var(--kn-surface-1)', borderBottom: '1px solid var(--kn-border)', padding: '0 24px', height: 56, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
      ...
      <div style={{ flex: 1, padding: '24px', maxWidth: 960, margin: '0 auto', width: '100%' }}>
        ...
        {showDonation && <DonationBanner isWinner={isOnWinningTeam} onDismiss={() => setShowDonation(false)} />}

        {/* 우승 팀 배너 */}
        {winner && (
          <div style={{ background: 'linear-gradient(135deg, color-mix(in oklab, var(--kn-accent) 12%, transparent) 0%, transparent 60%)', border: '1px solid color-mix(in oklab, var(--kn-accent) 40%, transparent)', borderRadius: 'var(--kn-r-xl)', padding: '28px 32px', marginBottom: 28, display: 'flex', alignItems: 'center', gap: 24, flexWrap: 'wrap' }}>
            <Icon name="trophy" size={48} color="var(--kn-accent)" />
            <div style={{ flex: 1 }}>
              <div data-label="" style={{ fontSize: 11, marginBottom: 4 }}>FINAL WINNER</div>
              <div style={{ fontSize: 36, fontWeight: 'var(--kn-w-black)', fontFamily: 'var(--kn-font-mono)', color: 'var(--kn-accent)', lineHeight: 1, marginBottom: 6 }}>{winner.name}</div>
            ...
            <div data-display="" style={{ fontSize: 72, color: 'var(--kn-accent)', fontFamily: 'var(--kn-font-mono)' }}>{winner.total}</div>
          </div>
        )}

        {/* MVP 배너 */}
        {mvp && mvp.kills > 0 && (
          <div style={{ background: 'var(--kn-surface-1)', border: '1px solid var(--kn-border)', borderRadius: 'var(--kn-r-lg)', padding: '16px 20px', marginBottom: 28, display: 'flex', alignItems: 'center', gap: 16 }}>
            <Icon name="zap" size={28} color="var(--kn-accent)" />
            <div>
              <div data-label="" style={{ fontSize: 10, marginBottom: 2 }}>MVP</div>
              <div style={{ fontSize: 18, fontWeight: 'var(--kn-w-bold)' }}>
                {mvp.nick}
            ...
            <section>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {teamScores.map((t, idx) => (
                  <div key={t.id} style={{ background: 'var(--kn-surface-1)', border: `1px solid ${idx === 0 ? 'color-mix(in oklab, var(--kn-accent) 40%, transparent)' : 'var(--kn-border)'}`, borderRadius: 'var(--kn-r-lg)', padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 16 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ display: 'flex', gap: 12, fontSize: 11, color: 'var(--kn-text-muted)', flexWrap: 'wrap' }}>
                        ...
                        {t.adj !== 0 && <span>수동조정 <b style={{ color: t.adj > 0 ? 'var(--kn-success)' : 'var(--kn-danger)' }}>{t.adj > 0 ? '+' : ''}{t.adj}</b></span>}
                      </div>
                    </div>
                    <div data-display="" style={{ fontSize: idx === 0 ? 40 : 32, color: idx === 0 ? 'var(--kn-accent)' : 'var(--kn-text)', fontFamily: 'var(--kn-font-mono)', minWidth: 60, textAlign: 'right' }}>
                      {t.total}
                    </div>
                  </div>
                ))}
              ...
              <div style={{ background: 'var(--kn-surface-1)', border: '1px solid var(--kn-border)', borderRadius: 'var(--kn-r-lg)', overflow: 'hidden' }}>
                {teamScores.map((team, tIdx) => {
                  const stats = (team.members || [])
                    .map((m) => ({ nick: m.playerNickname, kills: m.totalKills, bonus: m.bonusKills, penalty: m.penaltyKills, total: m.effectiveKills, damage: getPlayerDamage(m.playerNickname, matches) }))
                    .sort((a, b) => b.kills - a.kills);
                  return (
                    <div key={team.id}>
                      <div style={{ background: 'var(--kn-surface-2)', borderLeft: '3px solid var(--kn-accent)', padding: '8px 14px', fontSize: 12, fontWeight: 'var(--kn-w-bold)', borderTop: tIdx > 0 ? '1px solid var(--kn-border)' : 'none', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <span>{team.name}</span>
              ...
              <div style={{ background: 'var(--kn-surface-1)', border: '1px solid var(--kn-border)', borderRadius: 'var(--kn-r-lg)', padding: '12px 14px', maxHeight: 320, overflowY: 'auto' }}>
                {matches.length === 0 ? (
                  ...
                  [...matches].reverse().map((m) => {
                    ...
                    const teamKills = teamScores.map((t) => ({ name: t.name, kills: results.filter((r) => r.teamId === t.id).reduce((s, r) => s + r.kills, 0) }));
                    return (
                      <div key={m.matchId} style={{ padding: '10px 0', borderBottom: '1px solid var(--kn-border)' }}>
                        ...
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                          {teamKills.map((tk) => (
                            <div key={tk.name} style={{ background: 'var(--kn-surface-2)', border: '1px solid var(--kn-border)', borderRadius: 'var(--kn-r-sm)', padding: '3px 8px', fontSize: 11 }}>
            ...
            <section>
              ...
              <div style={{ background: 'var(--kn-surface-1)', border: '1px solid var(--kn-border)', borderRadius: 'var(--kn-r-lg)', padding: '14px 16px' }}>
                <div style={{ fontSize: 12, color: 'var(--kn-text-muted)', marginBottom: 8 }}>
                  목표 {rule.targetKills}킬 ·&nbsp;{rule.noTimeLimit ? '시간 제한 없음' : `${rule.timeLimitMin}분`}
                </div>
        ...
        <div style={{ marginTop: 32, display: 'flex', justifyContent: 'center', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
          {nextSession && (
            <div style={{ width: '100%', maxWidth: 480, background: 'color-mix(in oklab, var(--kn-accent) 8%, transparent)', border: '1px solid color-mix(in oklab, var(--kn-accent) 35%, transparent)', borderRadius: 'var(--kn-r-lg)', padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 14 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 14, fontWeight: 'var(--kn-w-bold)', color: 'var(--kn-text)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{nextSession.name}</div>
              </div>
              <Button variant="primary" size="sm" icon="arrow" onClick={() => router.push(`/room/${nextSession.roomCode}/setup`)} style={{ flexShrink: 0 }}>참여하기</Button>
            </div>
          )}
          {!nextSession && (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
              <Button variant="primary" size="lg" icon="zap" style={{ minWidth: 220 }} onClick={isHost ? handleRenew : undefined} loading={renewing} disabled={!isHost || renewing} title={isHost ? undefined : '호스트만 다음 킬내기를 시작할 수 있습니다'}>
                이어서 킬내기 시작
              </Button>
              {!isHost && (
                <div style={{ fontSize: 11, color: 'var(--kn-text-dim)', display: 'flex', alignItems: 'center', gap: 4 }}>
                  <Icon name="alert" size={12} /> 호스트가 다음 킬내기를 생성하면 알림이 표시됩니다
                </div>
              )}
              {renewError && (
          ...
          <Button variant="secondary" size="lg" onClick={() => router.push('/dashboard')} icon="back" style={{ minWidth: 200 }}>대시보드로 돌아가기</Button>
        </div>
      </div>
    </div>
  );
