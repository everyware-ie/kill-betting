...
- 즐겨찾기는 `domain/user`에 둔다 — `User`를 참조하므로 교차 모듈 엔티티 참조를 피한다.
- "최근 함께한 닉네임"(T7)은 저장하지 않고 조회 시 계산하는 **파생 데이터**다.
  팀 데이터는 team 모듈이 소유하므로 조회 쿼리도 `TeamService`에 두고 user 모듈이 소비한다.
- 즐겨찾기로 걸러낸 뒤에도 10개를 채우기 위해 team 조회는 상한보다 넉넉히 가져온다.
