...
approved FRD라도 착수 전 파이프라인(FRD 확인 → 이슈 분해 → 착수)을 건너뛰지 못하도록 두 지점에서 훅이 강제한다. 스텁(`docs/product/features/<기능>.md`)이 파이프라인 전 단계의 **단일 증거**이며, 착수 게이트가 검사하는 필수 필드(`REQUIRED_FIELDS`)로 강제 단계를 표현한다 — 현재 `FRD`(허브 approved 링크) + `이슈`(GitHub 이슈 번호 또는 `없음(소규모)`).

1. **구현 착수 시점** (`pre-implementation-frd-check.sh`): 구현 메인 소스(`backend/src/main`·`frontend/{app,components,features,lib}`)를 편집할 때, **이 브랜치에** 필수 필드를 모두 갖춘 확인 스텁이 없으면 편집이 차단된다. 테스트 파일(`*.test.js`·`*.spec.js`·`backend/src/test`)은 대상 아님(TDD test-first 허용), 브랜치 타입 `chore`·`docs`는 면제. 스텁을 갖추려면 허브 FRD를 가져와 보여주고 확인받은 뒤, 기능을 GitHub 이슈로 분해(소규모면 `없음(소규모)` 명시)해 스텁에 적어야 한다. **"바로 진행"으로도 이 단계는 건너뛸 수 없다.**
2. **PR 생성 시점** (`pre-pr-checklist.sh`): PR이 참조하는 허브 FRD(본문·`--body-file`·스텁 링크에서 수집)의 `status`가 `approved`가 아니면 차단. 또한 스텁이 참조한 `이슈: #N`이 실제 GitHub 이슈로 존재하는지 확인(존재하지 않으면 차단, 네트워크 장애 시 경고만).

훅이 강제하는 수준은 정직하게 다음까지다:
- 강제함: "이 브랜치에서 확인 체크포인트가 필수 필드(FRD·이슈)를 갖춰 실행됐다"
- 강제 못 함(범위 밖): 편집 중인 소스가 정확히 그 스텁의 기능인지(파일→기능 매핑), 사람이 실제로 정독·분해를 제대로 했는지 — 즉 파일 단위가 아니라 **브랜치 단위** 체크포인트다.
- **확장**: 1단계(요구사항 분석)까지 강제하려면 스텁에 `요구사항:` 필드를 켜고 `REQUIRED_FIELDS`에 `요구사항`을 추가하면 된다(새 아티팩트 불필요). 다만 과잉 강제는 피로·토큰 낭비이므로 적절한 선에서 결정한다.
