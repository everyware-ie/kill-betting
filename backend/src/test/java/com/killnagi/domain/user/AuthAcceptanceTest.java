...
class AuthAcceptanceTest extends AcceptanceTestSupport {
    ...
    void 이메일과_비밀번호로_회원가입하면_토큰을_발급받는다() {
        // Given
        String signUpBody = """
                {"nickname":"tester","email":"test@test.com","password":"password1!"}
                """;
    ...
    void 이미_사용_중인_이메일로_가입하면_실패한다() {
        ...
        post("/api/auth/signup", """
                {"nickname":"first","email":"%s","password":"password1!"}
                """.formatted(email));

        // When
        ResponseEntity<String> response = post("/api/auth/signup", """
                {"nickname":"second","email":"%s","password":"password1!"}
                """.formatted(email));
    ...
    void 가입한_이메일과_비밀번호로_로그인하면_토큰을_발급받는다() {
        // Given
        post("/api/auth/signup", """
                {"nickname":"tester","email":"login@test.com","password":"password1!"}
                """);
